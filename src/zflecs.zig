...
const EcsAllocator = struct {
    ...
    fn alloc(size: size_t) callconv(.c) ?*anyopaque {
        ...
        const data = gpa.alignedAlloc(u8, .fromByteUnits(Alignment), allocation_size) catch {
            return null;
        };
    ...
    fn realloc(old: ?*anyopaque, size: size_t) callconv(.c) ?*anyopaque {
        ...
        const new_data = gpa.realloc(old_slice_aligned, new_allocation_size) catch {
            return null;
        };
...
//
//--------------------------------------------------------------------------------------------------
pub fn init(gpa: std.mem.Allocator) *world_t {
    ...
    num_worlds += 1;
    world_component_lookup.put(EcsAllocator.gpa, world, .empty) catch @panic("OOM");
...
pub fn fini(world: *world_t) i32 {
    ...
    if (world_component_lookup.getPtr(world)) |component_type_lookup| {
        component_type_lookup.deinit(EcsAllocator.gpa);
    } else @panic("Uninitialised world");

    if (num_worlds == 0) {
        world_component_lookup.deinit(EcsAllocator.gpa);
        EcsAllocator.gpa = undefined;
    }
...
pub fn COMPONENT(world: *world_t, comptime T: type) void {
    ...
    component_type_lookup.put(EcsAllocator.gpa, perTypeGlobalId(T), component_id) catch @panic("OOM");
}
...
pub fn TAG(world: *world_t, comptime T: type) void {
    ...
    component_type_lookup.put(EcsAllocator.gpa, perTypeGlobalId(T), type_id) catch @panic("OOM");
}
